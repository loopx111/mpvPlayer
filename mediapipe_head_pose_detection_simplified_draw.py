...
def main():
    print("基于MediaPipe的头部姿态与注视检测 - 简化绘制版")
    print("优化特性：智能帧跳过 + 多人脸灰色线条简化绘制")
    print("=" * 70)
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            for i, face_landmarks in enumerate(results.multi_face_landmarks):
                ...
                # 智能绘制：根据人脸数量动态调整绘制细节
                face_info = smart_draw_landmarks(frame_flipped, face_landmarks, face_count, i, is_gazing, mp_drawing, mp_face_mesh)
            ...
            if frame_count % 30 == 0:
                ...
                print(f"[帧{frame_count}] {skip_info} 检测到{face_count}张人脸(灰色{draw_mode}绘制), {gazing_faces}张注视中, "
        ...
        # 在镜像后的帧上绘制人脸编号（只绘制关注中的人脸）
        if face_positions and results.multi_face_landmarks:
            face_count_display = len(results.multi_face_landmarks)
            for face_info in face_positions:
                if face_info['is_gazing']:
                    # 镜像后的人脸位置：水平镜像
                    mirrored_x_min = w - face_info['x_max']
                    mirrored_x_max = w - face_info['x_min']
                    
                    # 根据人脸数量选择编号位置
                    if face_count_display <= 2:
                        # 1-2张人脸：放在关键点图形右上角
                        text_x_pos = mirrored_x_max - 20
                        text_y_pos = face_info['y_min'] + 30
                    else:
                        # 3张及以上人脸：放在边界框左上角
                        text_x_pos = mirrored_x_min + 5
                        text_y_pos = face_info['y_min'] - 5
        ...
        # 修正Gaze Ratio计算逻辑：只统计当前检测帧的注视比例
        if results.multi_face_landmarks:
            # 当前帧有人脸，计算当前帧的注视比例
            current_gaze_ratio = (gazing_faces / len(results.multi_face_landmarks)) * 100
        else:
            # 当前帧无人脸，保持上一次的值
            current_gaze_ratio = gaze_ratio if 'gaze_ratio' in locals() else 0
        
        # 使用当前帧的注视比例
        gaze_ratio = current_gaze_ratio
        
        skip_rate = frame_optimizer.get_skip_rate()
        
        cv2.putText(display_frame, f"FPS: {int(fps)}", (text_x, 130), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        cv2.putText(display_frame, f"Inference: {display_inference_time}", (text_x, 150), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        ...
        cv2.imshow('MediaPipe Head Pose Detection - Simplified Draw', display_frame)
        ...
        if key == ord('q'):
            ...
            cv2.imwrite("mediapipe_head_pose_simplified.jpg", frame_flipped)
            print("保存当前帧到: mediapipe_head_pose_simplified.jpg")
