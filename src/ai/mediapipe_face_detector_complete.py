"""
MediaPipe人脸检测与注视识别控制器 - 完整移植版
直接移植自mediapipe_head_pose_detection_simplified_draw.py的所有功能
确保画面翻转、镜像处理和所有细节与测试脚本完全一致
"""
...
def smart_draw_landmarks(frame, face_landmarks, face_count, face_index, is_gazing, mp_drawing, mp_face_mesh):
    """
    智能绘制人脸关键点
    统一使用灰色线条绘制，无论人脸数量
    返回人脸位置信息用于后续绘制编号
    ...
    if face_count <= 2:
        ...
        mp_drawing.draw_landmarks(
            ...
            landmark_drawing_spec=mp_drawing.DrawingSpec(color=gray_color, thickness=1, circle_radius=1),
            connection_drawing_spec=mp_drawing.DrawingSpec(color=gray_color, thickness=1)
        )
    else:
        # 3张及以上人脸：简化绘制，使用灰色线条
        mp_drawing.draw_landmarks(
            frame,
            face_landmarks,
            mp_face_mesh.FACEMESH_CONTOURS,  # 仅轮廓，不是完整网格
            landmark_drawing_spec=None,  # 不绘制关键点
            connection_drawing_spec=mp_drawing.DrawingSpec(color=gray_color, thickness=1)
        )
...
class MediaPipeFaceDetector(QThread):
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # 执行完整的人脸检测流程
                analysis_result = self._complete_analyze_frame(frame)
    ...
    def _complete_analyze_frame(self, frame):
        """完整分析单帧图像 - 直接移植自测试脚本"""
        start_time_detect = time.time()
        
        # 重要：使用与测试脚本完全相同的帧进行检测
        # 测试脚本使用垂直翻转后的帧进行检测，确保关键点位置正确
        ...
        if should_skip and self.frame_optimizer.cached_results:
            ...
            inference_time = cached_inference_time  # 确保变量已定义
            ...
            if not current_frame_check.multi_face_landmarks:
                ...
                self.frame_optimizer.frame_skip_counter = 0
        ...
        # 记录推理时间
        if is_cached_result:
            display_inference_time = f"{cached_inference_time:.1f}ms"
            inference_time_val = cached_inference_time
        else:
            display_inference_time = f"{inference_time:.1f}ms"
            inference_time_val = inference_time
        
        self.inference_times.append(inference_time_val)
        self.frame_count += 1
        ...
        analysis_result = {
            ...
            'display_inference_time': display_inference_time,
            ...
            'face_positions': [],  # 用于绘制编号的人脸位置信息
            'drawn_frame': frame.copy()  # 保存原始帧，显示控件会处理镜像
        }
        ...
        if results.multi_face_landmarks:
            ...
            for i, face_landmarks in enumerate(results.multi_face_landmarks):
                ...
                analysis_result['face_positions'].append(face_info)
            ...
            if self.frame_count % 30 == 0:
                ...
                draw_mode = "详细" if face_count <= 2 else "简化"
                print(f"[帧{self.frame_count}] {skip_info} 检测到{face_count}张人脸(灰色{draw_mode}绘制), {gazing_faces}张注视中, "
                      f"推理时间: {display_inference_time}, 跳过率: {analysis_result['skip_rate']:.1f}%")
    ...
    def stop_analysis(self):
        ...
        print("MediaPipe人脸检测线程已停止（完整移植版）")


class MediaPipeCameraWidget(QtWidgets.QLabel):
    """MediaPipe摄像头显示控件 - 完整移植版"""
    ...
    def update_frame(self, frame, analysis_result=None):
        """更新显示的帧 - 完整移植自测试脚本的显示逻辑"""
        self.analysis_result = analysis_result
        
        if frame is not None:
            # 绘制分析结果
            display_frame = self._complete_draw_analysis_results(frame)
    ...
    def _complete_draw_analysis_results(self, frame):
        """在帧上绘制完整的分析结果 - 直接移植自测试脚本"""
        ...
        # 在镜像后的帧上绘制关键点和人脸编号
        if self.analysis_result['face_positions'] and self.analysis_result['faces']:
            face_count_display = len(self.analysis_result['faces'])
            
            # 绘制关键点图形（在水平镜像后的帧上）
            for face_info in self.analysis_result['face_positions']:
                h, w = display_frame.shape[:2]
                
                # 计算镜像后的人脸位置
                x_min = w - face_info['x_max']  # 镜像后x坐标需要翻转
                x_max = w - face_info['x_min']  # 镜像后x坐标需要翻转
                y_min = face_info['y_min']
                y_max = face_info['y_max']
                
                # 智能绘制关键点：根据人脸数量动态调整绘制细节
                ...
                if face_count_display <= 2:
                    ...
                    self.mp_drawing.draw_landmarks(
                        display_frame,
                        face_landmarks,
                        self.mp_face_mesh.FACEMESH_CONTOURS,  # 仅轮廓，不是完整网格
                        landmark_drawing_spec=None,  # 不绘制关键点
                        connection_drawing_spec=self.mp_drawing.DrawingSpec(color=gray_color, thickness=1)
                    )
                    
                    # 为每个人脸绘制边界框
                    cv2.rectangle(display_frame, (x_min, y_min), (x_max, y_max), gray_color, 2)
                ...
                if face_info['is_gazing']:
                    ...
                    # 只绘制绿色数字编号
                    face_num = face_info['face_index'] + 1
                    cv2.putText(display_frame, f"{face_num}", 
        ...
        if self.analysis_result['faces']:
            face_count_display = len(self.analysis_result['faces'])
            draw_mode = "Detail" if face_count_display <= 2 else "Simple"
            cv2.putText(display_frame, f"Draw Mode: {draw_mode}", (text_x, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(display_frame, f"Gazing: {self.analysis_result['gazing_faces']}", (text_x, 80), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        else:
            cv2.putText(display_frame, "Draw Mode: No Detect", (text_x, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(display_frame, "Gazing: 0", (text_x, 80), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        ...
        # 计算Gaze Ratio
        if self.analysis_result['faces']:
            gaze_ratio = (self.analysis_result['gazing_faces'] / len(self.analysis_result['faces'])) * 100
        else:
            gaze_ratio = 0
        
        cv2.putText(display_frame, f"FPS: {fps}", (text_x, 110), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        cv2.putText(display_frame, f"Inference: {self.analysis_result['display_inference_time']}", (text_x, 130), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        cv2.putText(display_frame, f"Skip Rate: {skip_rate:.1f}%", (text_x, 150), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
